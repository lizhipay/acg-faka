!function () {
    ...
    const STATUS = {
        0: {label: '待客服回复', className: 'is-waiting', icon: 'schedule', hint: i18n('客服查看后会在这里回复')},
        1: {label: '待我回复', className: 'is-reply', icon: 'mark_chat_unread', hint: i18n('补充信息后客服会继续处理')},
        2: {label: '已解决', className: 'is-resolved', icon: 'task_alt', hint: i18n('完整沟通记录已为你归档')},
        3: {label: '已关闭', className: 'is-closed', icon: 'lock', hint: i18n('历史沟通记录仍可随时查看')}
    };
    const SENDER = {
    ...
    function messageHtml(item) {
        ...
        const finalBadge = kind === 1 ? '<span class="uc-ticket-final"><span class="material-icons-outlined">task_alt</span>' + i18n('最终答复') + '</span>' : '';
        return `<article class="uc-ticket-message ${sender.className}" data-message-id="${number(item.id, 0)}">
            ...
            <div class="uc-ticket-message__main">
                <div class="uc-ticket-message__meta"><strong>${escapeHtml(name)}</strong>${sender.className === 'is-admin' ? '<span>' + i18n('官方客服') + '</span>' : ''}${finalBadge}<time>${escapeHtml(item.create_time || '')}</time></div>
    ...
    function renderRelated(ticket) {
        ...
        if (number(ticket.type, 0) === 0 && (ticket.commodity_name || context.commodity_name || context.commodity)) {
            ...
            html = `<div class="uc-ticket-related__item"><img src="${escapeHtml(cover)}" alt=""><span><small>${i18n('相关商品')}</small><strong>${escapeHtml(name)}</strong>${commodity.id ? `<em>${i18n('商品')} ID ${escapeHtml(commodity.id)}</em>` : ''}</span></div>`;
            ...
            html = `<div class="uc-ticket-related__item${guest ? ' is-guest' : ''}">${cover ? `<img src="${escapeHtml(cover)}" alt="">` : '<span class="uc-ticket-related__order-icon"><span class="material-icons-outlined">receipt_long</span></span>'}<span><small>${guest ? i18n('游客订单 · 待人工核验') : i18n('相关订单')}</small><strong>${escapeHtml(commodityName)}</strong><em>${escapeHtml(tradeNo || '')}${order.amount != null ? ` · ${formatMoney(order.amount)}` : ''}${order.pay_time || order.create_time ? ` · ${escapeHtml(order.pay_time || order.create_time)}` : ''}</em></span></div>`;
        }
    ...
    function updateTicketStatus(status) {
        ...
        $('.uc-ticket-live').toggleClass('is-archived', terminal).html(`<i></i>${terminal ? i18n('记录已归档') : i18n('自动更新')}`);
    ...
    function renderTicket(ticket) {
        ...
        $('.uc-ticket-detail-number').text(ticket.ticket_no || `${i18n('工单')} #${ticket.id}`);
        $('.uc-ticket-detail-title').text(ticket.title || i18n('未命名工单'));
    ...
    function submitReply() {
        ...
        $button.prop('disabled', true).addClass('is-loading').find('span:last').text(i18n('发送中…'));
        util.post({
            ...
            data: {id: ticketId, content: content},
                ...
                if (destroyed) return;
                replying = false;
                $button.prop('disabled', false).removeClass('is-loading').find('span:last').text(i18n('发送回复'));
    ...
    function showError(response) {
        ...
        $('.uc-ticket-detail-error p').text(response && response.msg ? response.msg : i18n('工单可能不存在，或你没有查看权限。'));
