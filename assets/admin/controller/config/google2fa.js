...
!function () {
    ...
    function copySecretFallback(secret, done) {
        ...
        try {
            ...
            else if (controllerActive) layer.msg(i18n('复制失败，请长按密钥手动复制'));
        } catch (error) {
            if (controllerActive) layer.msg(i18n('复制失败，请长按密钥手动复制'));
        }
        input.remove();
    }
    ...
    function refresh() {
        ...
        $('#g2fa-status').removeClass('text-danger').addClass('text-muted').text(i18n('正在读取绑定状态…'));
        ...
        util.post({
            ...
            loader: false,
                ...
                $('#g2fa-status').removeClass('text-danger text-muted').text(bound ? i18n('已绑定') : i18n('未绑定'));
            ...
            error: function (res) {
                if (!controllerActive) return;
                $('#g2fa-status').removeClass('text-muted').addClass('text-danger').text(res?.msg || i18n('绑定状态读取失败'));
                $('#g2fa-retry-btn').show();
            },
            fail: function () {
                if (!controllerActive) return;
                $('#g2fa-status').removeClass('text-muted').addClass('text-danger').text(i18n('网络异常，绑定状态读取失败'));
                $('#g2fa-retry-btn').show();
            }
        });
    }
    ...
    $('#g2fa-bind-btn').off(namespace).on('click' + namespace, function () {
        ...
        util.post("/admin/api/manage/googleSecret", {}, function (res) {
            ...
            component.popup({
                title: util.icon("fa-duotone fa-regular fa-shield-keyhole me-1") + i18n("绑定谷歌验证器"),
                width: "460px",
                height: "auto",
                autoPosition: true,
                confirmText: util.icon("fa-duotone fa-regular fa-floppy-disk me-1 text-success") + i18n("确认绑定"),
                tab: [{
                    name: "绑定谷歌验证器",
                    form: [
                        {
                            ...
                            complete: function (form, dom) {
                                ...
                                panel.append('<p class="md-g2fa-setup__hint mb-2 text-muted">' + i18n('用 Google / Microsoft Authenticator 扫码，或手动输入密钥') + '</p>');
                                const qr = $('<div class="md-g2fa-setup__qr d-inline-grid p-3 bg-white rounded-4" role="img" aria-label="' + i18n('谷歌验证器绑定二维码') + '"></div>');
                                const secretRow = $('<div class="md-g2fa-setup__secret d-flex flex-wrap align-items-center justify-content-center gap-2 mt-3"><span>' + i18n('密钥') + '</span><code></code><button type="button" class="btn btn-sm btn-light" aria-label="' + i18n('复制验证器密钥') + '">' + i18n('复制') + '</button></div>');
                                ...
                                try {
                                    ...
                                    qr.attr('aria-label', i18n('二维码生成失败，请使用下方密钥手动绑定')).append('<span class="text-danger">' + i18n('二维码生成失败，请手动输入密钥') + '</span>');
                                }
                            }
                        },
                        {
                            ...
                            required: true,
                            regex: {value: '^\\d{6}$', message: i18n('请输入 6 位数字动态码')}
                        }
                    ]
                }],
                ...
                submit: function (data, index) {
                    ...
                    util.post("/admin/api/manage/googleBind", {
                        ...
                        if (!controllerActive) return;
                        message.success(r.msg || i18n("绑定成功"));
                        layer.close(index);
                        if (reauthenticateIfNeeded(r)) return;
                        refresh();
                    }, function (r) {
                        bindSubmitting = false;
                        if (controllerActive) message.error(r?.msg || i18n('绑定失败'));
                    }, function () {
                        bindSubmitting = false;
            ...
            secretLoading = false;
            if (controllerActive) message.error(res?.msg || i18n('生成绑定密钥失败'));
        }, function () {
            secretLoading = false;
    ...
    $('#g2fa-unbind-btn').off(namespace).on('click' + namespace, function () {
        component.popup({
            title: util.icon("fa-duotone fa-regular fa-link-slash me-1") + i18n("解绑谷歌验证器"),
            width: "420px",
            height: "auto",
            autoPosition: true,
            danger: true,
            confirmText: util.icon("fa-duotone fa-regular fa-trash-can me-1 text-danger") + i18n("确认解绑"),
            tab: [{
                name: "解绑谷歌验证器",
                form: [
                    { title: "动态码", name: "code", type: "input", placeholder: "输入当前 6 位动态码", required: true, regex: {value: '^\\d{6}$', message: i18n('请输入 6 位数字动态码')} }
                ]
            }],
            renderComplete: prepareCodeInput,
            submit: function (data, index) {
                if (unbindSubmitting) {
                    layer.msg(i18n('正在解绑，请稍候'));
                    return;
                }
                ...
                util.post("/admin/api/manage/googleUnbind", { code: (data.code || '').trim() }, function (r) {
                    ...
                    if (!controllerActive) return;
                    message.success(r.msg || i18n("已解绑"));
                    layer.close(index);
                    if (reauthenticateIfNeeded(r)) return;
                    refresh();
                }, function (r) {
                    unbindSubmitting = false;
                    if (controllerActive) message.error(r?.msg || i18n('解绑失败'));
                }, function () {
                    unbindSubmitting = false;
